...
int main(int argc, char **argv)
{
  ...
  image_transport::ImageTransport its(n);
  image_transport::Subscriber sub_img0 = its.subscribe("/rgb/image_raw", 100, &ImageGrabber::GrabImage,&igb,image_transport::TransportHints("compressed"));
  std::thread sync_thread(&ImageGrabber::SyncWithImu,&igb);

/*
  image_transport::ImageTransport its(n);
  message_filters::Subscriber<Imu> sub_imu(n, "/imu", 1000);
  message_filters::Subscriber<Image> sub_img0(its, "/rgb/image_raw", 100);
  ...
  sync.registerCallback(boost::bind(&ImageGrabber::SyncWithImu, &igb));
  */
  ros::spin();
